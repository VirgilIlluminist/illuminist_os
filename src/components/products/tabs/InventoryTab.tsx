    ...
    <div className="space-y-4">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <p className="text-xs uppercase tracking-widest text-[var(--color-text-muted)]">Riwayat Pergerakan</p>
          <div className="flex gap-1">
            {(['all', 'in', 'out'] as Filter[]).map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-2 py-0.5 rounded-lg text-xs uppercase cursor-pointer transition-all ${filter === f ? 'text-black' : 'text-[var(--color-text-muted)] bg-white/5'}`}
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-8 text-xs text-[var(--color-text-muted)]">Tidak ada pergerakan stok.</div>
        ) : (
