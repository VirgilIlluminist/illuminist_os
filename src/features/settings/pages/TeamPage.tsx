...
export default function TeamPage() {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      {/* Invite form */}
      <div className="glass-panel p-5 bg-[var(--color-card-bg)] border border-[var(--color-border-line)] rounded-xl space-y-4">
        ...
        <div className="flex gap-3 flex-wrap">
          <input
            ...
            className="flex-1 min-w-48 px-4 py-2.5 text-sm bg-white/[0.06] border border-white/[0.10] text-white rounded-xl focus:outline-none transition-colors"
          />
          <select
            value={invRole}
            onChange={e => setInvRole(e.target.value)}
            className="px-4 py-2.5 text-sm bg-white/[0.06] border border-white/[0.10] text-white rounded-xl focus:outline-none cursor-pointer"
          >
            {ROLES.map(r => (
          ...
          <button
            ...
            className="px-4 py-2.5 text-sm font-bold uppercase tracking-wider rounded-xl transition-all cursor-pointer disabled:opacity-40"
      ...
      {/* SQL helper */}
      <div className="glass-panel p-4 bg-[var(--color-card-bg)] border border-[var(--color-border-line)] rounded-xl">
        <p className="text-sm font-medium text-white/50 mb-2 block uppercase tracking-wide">
          SQL untuk set role setelah user dibuat:
        </p>
        <pre className="text-xs text-emerald-400 bg-black/30 border border-white/[0.08] rounded-xl p-4 overflow-x-auto">
...
WHERE id = (SELECT id FROM auth.users WHERE email = 'email@karyawan.com');`}
      ...
      {/* Members list */}
      <div className="glass-panel p-5 bg-[var(--color-card-bg)] border border-[var(--color-border-line)] rounded-xl">
            ...
            {members.map(m => (
              ...
              >
                ...
                <select
                  ...
                  className="text-sm px-3 py-1.5 bg-white/[0.06] border border-white/[0.10] text-white rounded-xl cursor-pointer focus:outline-none"
                >
                  {ROLES.map(r => (
                ...
                >
                  {m.is_active ? <Trash2 size={14} /> : <Eye size={14} />}
                </button>
              </div>
            ))}
          </div>
